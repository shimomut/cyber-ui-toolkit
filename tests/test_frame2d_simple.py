#!/usr/bin/env python3
"""
Test Frame2D with a simple rectangle to isolate the rendering issue.
"""

import sys
sys.path.insert(0, '../build')

import cyber_ui_core as ui
from PIL import Image as PILImage
import os

def main():
    print("=== Frame2D Simple Test ===\n")
    
    # Initialize renderer
    renderer = ui.create_metal_renderer()
    if not renderer.initialize(800, 700, "Frame2D Simple Test"):
        print("Failed to initialize renderer")
        return
    
    print("✓ Renderer initialized\n")
    
    # Create scene
    scene = ui.SceneRoot()
    camera = scene.get_camera()
    camera.set_position(0.0, 0.0, 800.0)
    camera.set_perspective(1.0472, 800.0/700.0, 0.1, 2000.0)
    
    # Create Frame3D
    frame3d = ui.Frame3D()
    frame3d.set_position(0.0, 0.0, 0.0)
    scene.add_frame3d(frame3d)
    
    # Create Frame2D
    print("Creating Frame2D...")
    frame2d = ui.Frame2D()
    frame2d.set_position(0.0, 0.0)  # At origin
    frame2d.set_size(400.0, 400.0)
    frame2d.set_clipping_enabled(True)
    print(f"  Position: (0, 0)")
    print(f"  Size: (400, 400)")
    print(f"  Clipping: ENABLED")
    
    # Add a simple rectangle to Frame2D
    print("\nAdding rectangle to Frame2D...")
    rect = ui.Rectangle(200.0, 200.0)
    rect.set_position(0.0, 0.0)  # Relative to Frame2D
    rect.set_color(1.0, 0.0, 0.0, 1.0)  # Red
    frame2d.add_child(rect)
    print(f"  RED rectangle at (0, 0) size 200x200")
    
    frame3d.add_child(frame2d)
    
    print()
    
    # Render
    if renderer.begin_frame():
        renderer.render_scene(scene)
        renderer.end_frame()
    
    # Capture
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "output")
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "frame2d_simple_test.png")
    
    if renderer.save_capture(output_file):
        print(f"✓ Saved to: {output_file}\n")
        
        # Analyze
        img = PILImage.open(output_file)
        pixels = list(img.getdata())
        
        bg_color = (26, 26, 26, 255)
        non_bg_count = sum(1 for p in pixels if abs(p[0] - bg_color[0]) > 30 or 
                          abs(p[1] - bg_color[1]) > 30 or 
                          abs(p[2] - bg_color[2]) > 30)
        
        print(f"Non-background pixels: {non_bg_count} ({non_bg_count/len(pixels)*100:.2f}%)")
        
        if non_bg_count > 0:
            print("✓ Rectangle is visible!")
        else:
            print("✗ Rectangle NOT visible - Frame2D rendering is broken")
    
    renderer.shutdown()

if __name__ == "__main__":
    main()
